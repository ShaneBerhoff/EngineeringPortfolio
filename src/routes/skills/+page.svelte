<script lang="ts">
	...
	let { data }: { data: SkillsConfig } = $props();
</script>

<div class="flex h-full w-full justify-center">
	<div class="flex h-full max-w-5xl flex-grow flex-col">
		<div class="p-4 text-center text-4xl sm:text-6xl font-bold">Skills</div>
		{#each data.section as section}
			<div class="flex flex-row items-center gap-2 p-4">
				<hr class="sm:w-12 border-gray-300" />
				<div class="text-sm text-gray-500">{section.name}</div>
				<hr class="flex-1 border-gray-300" />
			</div>
			<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-10 px-4">
				{#each section.skill as skill}
